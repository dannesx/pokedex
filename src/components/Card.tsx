...
function Card({ pokemon }: { pokemon: Pokemon }) {
	...
	return (
		<div id={pokemon.name} className={`bg-${mainType}bg p-4 rounded-xl relative shadow-lg mb-8 animate-fadeIn`}>
			...
			<img src={sourceImg} alt={pokemon.name} className='absolute max-w-32 bottom-1 right-1 z-50'/>
			<img src="/assets/patterns/6x3.svg" alt="6x3 pattern" className='absolute top-2 left-24'/>
			<img src="/assets/patterns/pokeball-card.svg" alt="pokeball pattern" className='absolute top-0 right-0 '/>
		</div>
	)
}
export default Card
